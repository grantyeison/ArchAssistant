...
public class PatronJPADAO implements PatronDAO
{
    ...
    /*
    @Override
    public List<tacticapatronList> ListarTacticasDePatron(int id) {
        System.out.println("patron para consultar ID= " + id);
        Patron patron = this.BuscarPatron(id);
        if (patron != null) {
            System.out.println("el patron elegido es ()+ " + patron + " y sus tacticas asociadas son" + patron.getTacticapatronList());
            return patron.get(tacticap);
        }else   return null;
    }*/
}
